...
/**
 * The number box contains a label and two buttons.
 * The displayed number can be altered using the buttons.
	...
	public GuiNumberbox(float x, float y, float width, float height)
	{
		...
		plus.setEvent(new GuiEvent(){
			...
			public void run(GuiEventType eventtype, GuiElement element) {
				...
				switch (eventtype) {
				case Click:
					if(n.getValue()+n.getStep()<=n.getMax())n.setValue(n.getValue()+n.getStep());
		...
		minus.setEvent(new GuiEvent(){
			...
			public void run(GuiEventType eventtype, GuiElement element) {
				...
				switch (eventtype) {
				case Click:
					if(n.getValue()-n.getStep()>=n.getMin())n.setValue(n.getValue()-n.getStep());
					n.callGuiEvents(GuiEventType.Valuechange);
					break;
				default:
					break;
